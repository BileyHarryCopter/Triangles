...
{
    return Comparison::are_equal (distance (p, pl), 0.0);
}

//  block with Plane and Line
bool is_intersect (const Line &line, const Plane &pl)
{
    return Comparison::are_equal (scalar_product (pl.norm_vec (), line.drc_vec_), 0.0);
}

bool are_paral (const Plane &pl, const Line &line)
{
    return are_collinear (pl.norm_vec (), line.drc_vec_);
}
bool are_paral (const Line &line, const Plane &pl) { return are_paral (pl, line); }

bool is_consisted (const Line &line, const Plane &pl)
{
    return Comparison::are_equal (pl.A_ * line.point_.x_ + pl.B_ * line.point_.y_ +
                                      pl.C_ * line.point_.z_ + pl.D_,
                                  0.0) &&
           Comparison::are_equal (
               pl.A_ * line.drc_vec_.x_ + pl.B_ * line.drc_vec_.y_ + pl.C_ * line.drc_vec_.z_, 0.0);
}
...
Point p_intersect (const Line &line, const Plane &pl)
{
    ...
    return Point p_inrs = {line.point_.x_ + line.drc_vec_.x_ * t + line.point_.y_ +
                           line.drc_vec_.y_ * t + line.point_.z_ + line.drc_vec_.z_ * t};
}
...
{
    return are_paral (pl1, pl2) && Comparison::are_equal (pl1.D_, pl2.D_);
}
